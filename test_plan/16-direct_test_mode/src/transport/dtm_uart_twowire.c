...
/* Auto-end timer for TX test: automatically ends TX test after 10 seconds */
#define TX_TEST_DURATION_SEC 10

static volatile bool tx_test_timeout;

static void tx_timeout_cb(struct k_timer *timer)
{
	tx_test_timeout = true;
}

static K_TIMER_DEFINE(tx_auto_timer, tx_timeout_cb, NULL);
...
/* Debug output via UART, reusable by dtm.c */
#define DBG_BUF_SIZE 128
static char dbg_buf[DBG_BUF_SIZE];

static void dtm_dbg(const char *s)
{
	uart_poll_out(dtm_uart, '\r');
	uart_poll_out(dtm_uart, '\n');
	while (*s) {
		uart_poll_out(dtm_uart, *s++);
	}
}
...
static int reset_dtm(uint8_t parameter)
{
	...
	k_timer_stop(&tx_auto_timer);
	tx_test_timeout = false;
...
/* Default total number of packets sent for packet loss rate calculation */
#define DTM_TOTAL_TX_PACKETS 1000
...
static uint16_t on_test_end_cmd(enum dtm_ctrl_code control, uint8_t parameter)
{
	...
	/* Print packet loss rate via UART (same channel as response packets) */
	if (!err) {
		uint32_t lost = (DTM_TOTAL_TX_PACKETS > cnt) ?
				(DTM_TOTAL_TX_PACKETS - cnt) : 0;
		uint32_t loss_rate_x100 = (lost * 10000) / DTM_TOTAL_TX_PACKETS;

		snprintf(dbg_buf, DBG_BUF_SIZE,
			 "[DTM] RX: %u/%u, lost: %u, loss rate: %u.%02u%%",
			 cnt, DTM_TOTAL_TX_PACKETS, lost,
			 loss_rate_x100 / 100, loss_rate_x100 % 100);
		dtm_dbg(dbg_buf);
	}
...
static uint16_t on_test_tx_cmd(uint8_t chan, uint8_t length, enum dtm_pkt_type type)
{
	...
	if (!err) {
		tx_test_timeout = false;
		k_timer_start(&tx_auto_timer, K_SECONDS(TX_TEST_DURATION_SEC), K_NO_WAIT);
	}
...
{
	LOG_ERR("Waiting for DTM command.");
	...
	for (;;) {
		...
		/* Check if TX test auto-end timer has expired */
		if (tx_test_timeout) {
			tx_test_timeout = false;
			LOG_INF("TX test auto-end after %d seconds", TX_TEST_DURATION_SEC);
			tmp.twowire = 0xC000; /* Inject TEST_END command */
			return tmp;
		}
