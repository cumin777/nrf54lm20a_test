...
/* Debug output via DTM UART (disabled) */
// extern void dtm_dbg(const char *s);
...
static bool check_pdu(const struct dtm_pdu *pdu)
{
	...
	if (length > DTM_PAYLOAD_MAX_SIZE) {
		// snprintf(dbg_buf, DBG_BUF_SIZE, "[4a] FAIL: bad len=%u", length);
		// dtm_dbg(dbg_buf);
		return false;
	}
	...
	if ((dtm_inst.radio_mode == NRF_RADIO_MODE_BLE_1MBIT ||
		...
		// snprintf(dbg_buf, DBG_BUF_SIZE, "[4a] FAIL: bad type=%u for 1M/2M", pdu_packet_type);
		// dtm_dbg(dbg_buf);
		return false;
	}
	...
	if (dtm_hw_radio_lr_check(dtm_inst.radio_mode) &&
		...
		// snprintf(dbg_buf, DBG_BUF_SIZE, "[4a] FAIL: bad type=%u for coded", pdu_packet_type);
		// dtm_dbg(dbg_buf);
		return false;
	}
	...
	case DTM_PDU_TYPE_PRBS9: {
		bool match = (memcmp(payload, dtm_prbs9_content, length) == 0);
		return match;
	}
	...
	for (uint8_t k = 0; k < length; k++) {
		...
		if (pdu->content[k + 2] != pattern) {
			...
			// 	"[4b] FAIL: pattern[%u]=%02X want=%02X",
			// 	k, pdu->content[k + 2], pattern);
			// dtm_dbg(dbg_buf);
			return false;
		}
	}
...
int dtm_test_end(uint16_t *pack_cnt)
{
	...
	if (dtm_inst.state == STATE_TRANSMITTER_TEST) {
		*pack_cnt = dtm_inst.tx_pkt_count;
	} else {
...
static void on_radio_end_event(void)
{
	...
	// snprintf(dbg_buf, DBG_BUF_SIZE, "[3] CRC: %s", crc_ok ? "PASS" : "FAIL");
	// dtm_dbg(dbg_buf);
	// snprintf(dbg_buf, DBG_BUF_SIZE, "[3] HDR: %02X %02X",
	// 	received_pdu->content[0], received_pdu->content[1]);
	// dtm_dbg(dbg_buf);
	// snprintf(dbg_buf, DBG_BUF_SIZE, "[3] DATA: %02X %02X %02X %02X %02X %02X %02X %02X",
	// 	received_pdu->content[2], received_pdu->content[3],
	// 	received_pdu->content[4], received_pdu->content[5],
	// 	received_pdu->content[6], received_pdu->content[7],
	// 	received_pdu->content[8], received_pdu->content[9]);
	// dtm_dbg(dbg_buf);

	// bool pdu_ok = check_pdu(received_pdu);
	// snprintf(dbg_buf, DBG_BUF_SIZE, "[4] PDU: %s", pdu_ok ? "PASS" : "FAIL");
	// dtm_dbg(dbg_buf);
...
static void radio_handler(const void *context)
{
	if (nrf_radio_event_check(NRF_RADIO, NRF_RADIO_EVENT_ADDRESS)) {
		...
		// snprintf(dbg_buf, DBG_BUF_SIZE, "[1] AA matched, state=%u", dtm_inst.state);
		// dtm_dbg(dbg_buf);
#if NRF52_ERRATA_172_PRESENT
		if (dtm_inst.state == STATE_RECEIVER_TEST &&
	...
	if (nrf_radio_event_check(NRF_RADIO, NRF_RADIO_EVENT_END)) {
		...
		// dtm_dbg("[2] END event: pkt received");

		on_radio_end_event();

		/* Count transmitted packets */
		if (dtm_inst.state == STATE_TRANSMITTER_TEST) {
			dtm_inst.tx_pkt_count++;
		}
	}
